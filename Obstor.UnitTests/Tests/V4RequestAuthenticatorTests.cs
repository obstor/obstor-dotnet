...
public class V4RequestAuthenticatorTests
{
    ...
    public async Task ValidateAuthentication()
    {
        ...
        Assert.Equal("Credential=obstoradmin/20240411/us-east-1/s3/aws4_request, SignedHeaders=host;x-amz-content-sha256;x-amz-date, Signature=fbc9b67904568217c4dcdd438483fa7ff914a793e532d215eecddae7f78bdfe8", req.Headers.Authorization.Parameter);
    }
    ...
    public async Task ValidateAuthenticationWithComplexQuery()
    {
        ...
        Assert.NotNull(req.Headers.Authorization!.Scheme);
        Assert.Equal("AWS4-HMAC-SHA256", req.Headers.Authorization.Scheme);
        Assert.Equal("Credential=obstoradmin/20240411/us-east-1/s3/aws4_request, SignedHeaders=host;x-amz-content-sha256;x-amz-date, Signature=3c80a4a86d5c03a9978ff5f125b40fc75ed2fd27bf7c8ef26dec185b6ea63f44", req.Headers.Authorization.Parameter);
    }
    ...
    public async Task ValidateAuthenticationWithEmptyQueryValue()
    {
        ...
        Assert.NotNull(req.Headers.Authorization!.Scheme);
        Assert.Equal("AWS4-HMAC-SHA256", req.Headers.Authorization.Scheme);
        Assert.Equal("Credential=obstoradmin/20240411/us-east-1/s3/aws4_request, SignedHeaders=host;x-amz-content-sha256;x-amz-date, Signature=71b660a420c73e3b080c890af76dcf86019e09fa0a73a3b79526c8bf6ca340b7", req.Headers.Authorization.Parameter);
    }
    ...
    private static async Task AuthenticateRequestAsync(HttpRequestMessage req)
    {
        ...
        {
            AccessKey = "obstoradmin",
            SecretKey = "obstoradmin",
        }));
