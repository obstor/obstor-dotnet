...
public class RetryTests : ObstorUnitTests
{
    ...
    public async Task CheckRetries(HttpStatusCode statusCode, bool expectRetry)
    {
        ...
        async obstorClient =>
        {
            if (expectRetry)
            {
                var exists = await obstorClient.BucketExistsAsync("testbucket").ConfigureAwait(true);
                Assert.True(exists);
            }
            else
            {
                try
                {
                    await obstorClient.BucketExistsAsync("testbucket").ConfigureAwait(true);
                    Assert.Fail("Should throw exception");
                }
                catch (ObstorHttpException exc)
                {
