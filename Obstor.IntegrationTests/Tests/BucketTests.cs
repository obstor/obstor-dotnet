...
public class BucketTests : ObstorTest
{
    [Fact]
    ...
    public async Task BucketEncryptionWithoutConfig()
    {
        ...
        // A full Set/Get/Remove round-trip is not covered here because Obstor requires a KMS
        // backend to enable bucket-default SSE; the standard Testcontainers image has none.
        var ex = await Assert.ThrowsAsync<ObstorHttpException>(
    ...
    public async Task BucketEncryptionSetThrowsNotImplemented()
    {
        ...
        // Obstor requires a KMS backend (e.g. KES/Vault) to enable bucket-default SSE, regardless
        // of algorithm. The standard Testcontainers Obstor image has no KMS configured, so the server
        // returns HTTP 501 Not Implemented. Verify the SDK surfaces this as a ObstorHttpException.
