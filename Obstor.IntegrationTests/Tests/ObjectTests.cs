...
public class ObjectTests : ObstorTest
{
    ...
    public async Task TestPutAndGetObject()
    {
        ...
        {
            ContentType = new MediaTypeHeaderValue("application/x-obstor-random-testdata"),
            UserMetadata =
            {
    ...
    public async Task TestListObjects1()
    {
        ...
        Assert.Equal("Obstor-test", stream.Info.UserMetadata["FixedKey"]);
        ...
        await foreach (var objItem in client.ListObjectsAsync(BucketName, prefix: "test-", delimiter: "/", includeMetadata: true, pageSize: 10))
        {
            ...
            Assert.Equal("Obstor-test", objItem.UserMetadata["FixedKey"]);
    ...
    public async Task TestListMultipartUpload()
    {
        ...
        // IMPORTANT: In Obstor this call is only able to list when a prefix is set to an exact object-name
    ...
    [Fact(Skip = "Community Obstor does not return delete errors for invalid ETag")]
