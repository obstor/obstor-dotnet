...
/// The access key is resolved from <c>OBSTOR_ROOT_USER</c>, <c>OBSTOR_ACCESS_KEY</c>, or
/// <c>AWS_ACCESS_KEY_ID</c> (in that order of precedence). The secret key is resolved from
/// <c>OBSTOR_ROOT_PASSWORD</c>, <c>OBSTOR_SECRET_KEY</c>, or <c>AWS_SECRET_ACCESS_KEY</c>.
...
public class EnvironmentCredentialsProvider : ICredentialsProvider
{
    ...
    {
        var accessKey = GetEnvironmentString("OBSTOR_ROOT_USER", "OBSTOR_ACCESS_KEY", "AWS_ACCESS_KEY_ID");
        var secretKey = GetEnvironmentString("OBSTOR_ROOT_PASSWORD", "OBSTOR_SECRET_KEY", "AWS_SECRET_ACCESS_KEY");
