...
namespace Obstor.Implementation;

internal class ObstorClient : IObstorClient
{
    ...
    private readonly ILogger<ObstorClient> _logger;
    private int _requestId;

    public ObstorClient(IOptions<ClientOptions> options, ITimeProvider timeProvider, IRequestAuthenticator requestAuthenticator, IHttpClientFactory httpClientFactory, ILogger<ObstorClient> logger)
    {
        _options = options;
    ...
    public async Task<bool> BucketExistsAsync(string bucketName, CancellationToken cancellationToken)
    {
        ...
        catch (ObstorHttpException exc) when (exc.Response.StatusCode == HttpStatusCode.NotFound)
        {
            return false;
        }
    }
    ...
    public async Task<IDictionary<string, string>?> GetBucketTaggingAsync(string bucketName, CancellationToken cancellationToken = default)
    {
        ...
        catch (ObstorHttpException obstorHttpException) when (obstorHttpException.Error?.Code == "NoSuchTagSet")
        {
                    ...
                    if (includeMetadata)
                    {
                        ...
                        if (xUserMetadata == null)
                            throw new InvalidOperationException(
                                "Client doesn't support metadata while listing objects (Obstor specific feature)");
    ...
    public async Task<ObjectLockConfiguration> GetObjectLockConfigurationAsync(string bucketName, CancellationToken cancellationToken = default)
    {
        ...
        catch (ObstorHttpException exc) when (exc.Error?.Code == "ObjectLockConfigurationNotFoundError")
        {
    ...
    public async Task<LifecycleConfiguration?> GetBucketLifecycleAsync(string bucketName, CancellationToken cancellationToken)
    {
        ...
        catch (ObstorHttpException exc) when (exc.Error?.Code == "NoSuchLifecycleConfiguration")
        {
            return null;
        }
    }
    ...
    public async Task<ReplicationConfiguration?> GetBucketReplicationAsync(string bucketName, CancellationToken cancellationToken)
    {
        ...
        catch (ObstorHttpException exc) when (exc.Error?.Code == "ReplicationConfigurationNotFoundError")
        {
            return null;
        }
    }
    ...
    public async Task<string?> GetPolicyAsync(string bucketName, CancellationToken cancellationToken)
    {
        ...
        catch (ObstorHttpException exc) when (exc.Error?.Code == "NoSuchBucketPolicy")
        {
            return null;
        }
    }
    ...
    public async Task<IDictionary<string, string>?> GetObjectTagsAsync(string bucketName, string key, string? versionId, CancellationToken cancellationToken)
    {
        ...
        catch (ObstorHttpException exc) when (exc.Error?.Code == "NoSuchTagSet")
        {
            return null;
        }
    }
    ...
    private async Task<HttpResponseMessage> SendRequestAsync(HttpRequestMessage req, CancellationToken cancellationToken)
    {
        ...
        using var httpClient = _httpClientFactory.CreateClient(_options.Value.ObstorHttpClient);
        ...
        catch (Exception exc) when (exc is not ObstorHttpException)
        {
            if (_logger.IsEnabled(LogLevel.Debug))
